...
function migrate(raw){
  ...
  raw.frames.forEach(f=>{
    ...
    f.lines=(f.lines||[]).map(l=>{
      if(Array.isArray(l.points)){if(typeof l.manualCurve!=='boolean')l.manualCurve=true;if(typeof l.targetKey==='undefined')l.targetKey=null;return l;}
      const cx=Number.isFinite(l.cx)?l.cx:(l.x1+l.x2)/2,cy=Number.isFinite(l.cy)?l.cy:(l.y1+l.y2)/2;
      return{type:l.type==='move'?'move':l.type||'move',sourceKey:l.sourceKey||null,targetKey:l.targetKey||null,manualCurve:true,points:[{x:l.x1,y:l.y1},{x:cx,y:cy},{x:l.x2,y:l.y2}]};
    });
