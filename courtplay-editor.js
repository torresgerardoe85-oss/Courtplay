...
function createActionFromSelectedPlayer(type){const pl=frame().players.find(p=>p.key===selectedPlayer);if(!pl){setStatus('Primero selecciona un jugador.');return;}const dirX=pl.x<W/2?1:-1;let end={x:clamp(pl.x+dirX*115,45,W-45),y:clamp(pl.y-105,45,H-45)};if(type==='screen')end={x:clamp(pl.x+dirX*80,45,W-45),y:clamp(pl.y-80,45,H-45)};if(type==='handoff')end={x:clamp(pl.x+dirX*95,45,W-45),y:pl.y};if(type==='shot')end={x:500,y:112};const mid={x:(pl.x+end.x)/2,y:(pl.y+end.y)/2};const action={type,sourceKey:pl.key,points:[{x:pl.x,y:pl.y},mid,end]};frame().lines.push(action);selectedLine=frame().lines.length-1;tool='select';document.querySelectorAll('.tool').forEach(b=>b.classList.remove('active'));setStatus(`${actionName(type)} creado desde jugador ${pl.label}. Punta verde = destino. Punto naranja = curva.`);render();}
